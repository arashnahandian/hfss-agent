...
def test_every_gate_finding_carries_hfss_text_only_on_its_provenance() -> None:
    ...
    text are the five selection names on ``provenance``. None of the eleven
    free-text fields on ``Finding`` itself carries any -- they are wrapper
    constants and wrapper-composed prose.
    ...
    arm. Then W-10 would be handing on HFSS text through fields nobody measured,
    and this fires.
    """
    findings = _gate_findings_from_a_hostile_session()
    ...
    for finding in findings:
        ...
        for name in Finding.model_fields:
            ...
            if isinstance(value, str):
                assert INSTRUCTION not in value, f"{finding.rule_id}.{name}"
                assert "SSSSS" not in value, f"{finding.rule_id}.{name}"
...
def test_the_rendering_is_byte_deterministic() -> None:
    ...
    FAILS IF: a set, an unordered dict iteration, or a sort enters the render
    path -- and a sort would be worse than nondeterministic, because keyed on an
    engine-authored id it would make the output depend on untrusted content.
    """
    receipt = merge_findings(
        gate_findings=accepted_gate_findings(),
...
def test_the_renderer_reads_no_field_that_is_not_on_a_finding() -> None:
    ...
    Four fields are rendered per accepted finding and seven are deliberately not
    -- see ``_accepted_entry`` for why evidence belongs on the receipt rather
    than in prose. Widening this is a reasonable thing to want and should be a
    reviewed diff: rendering ``observed_values`` would put engine-authored KEYS
    into the text, which is a new untrusted surface.

    FAILS IF: a field is added to or removed from the rendered entry.
    """
